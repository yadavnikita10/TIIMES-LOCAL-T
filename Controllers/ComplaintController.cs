...
namespace TuvVision.Controllers
{
    public class ComplaintController : Controller
    {
        ...
        public ActionResult Delete(int PK_Complaint_ID)
        {
            int Result = 0;
            try
            {
                Result = objDAM.DeleteComplaint(PK_Complaint_ID);
                if (Result != 0)
                {
        ...
        public JsonResult getAutoControlno(string input)
   {
        ...
        public ActionResult Download2(string p, int d, string name)
        {
            // Strip extension if user passed it in query string
            //string fileId = Path.GetFileNameWithoutExtension(p);

            // Build path where the file is actually saved
            string filePath = Path.Combine(Server.MapPath("~/Content/JobDocument"), p);
            ...
            // Use the original filename passed from the link
            string downloadName = string.IsNullOrEmpty(name) ? "DownloadedFile" : name;

            // Return the file with the correct name
